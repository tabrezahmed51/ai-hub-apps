...
constexpr uint32_t c_default_image_height = 224;
constexpr uint32_t c_default_image_width = 224;
...
void PrintHelp()
{
    ...
    std::cout << "./Classification.exe --backend npu " << c_option_model_path << "<model_path> " << c_option_image_path
              << " <input image path>\n";
    std::cout << "\n::::::::Classification App options::::::::\n";
    std::cout << "\nRequired options:\n\n";
    ...
    std::cout << c_option_image_path
              << " <local_path>: [Required] Path to local input "
                 "image to run Classification app on.\n";
    std::cout << "\nOptional options:\n\n";
    std::cout << "--backend <backend>: Default = npu. Set backend for inference. "
                 "Available options: cpu, npu.\n";
    std::cout << "--model_input_ht <model_input_ht>: Default = 224. Input "
                 "spatial height expected by model.\n";
    std::cout << "--model_input_wt <model_input_wt>: Default = 224. Input "
...
int main(int argc, char* argv[])
{
    ...
    std::string backend, model_path, image_path;
    ...
    try
    {
        App::ClassificationApp app(model_path, input_image_height, input_image_width);
