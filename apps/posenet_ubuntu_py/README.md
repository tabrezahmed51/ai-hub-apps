# Posenet Pose Estimation app

A Python app using GStreamer, OpenCV, and LiteRT that performs multi-person
human pose estimation on a live camera stream.
...
qai-hub-apps fetch posenet_ubuntu_py --model posenet_mobilenet --output-dir ~
cd ~/posenet_ubuntu_py
```
...
>
> ```bash
> qai-hub-apps fetch posenet_ubuntu_py --model <path/to/exported_model>
> ```
...
- `models/posenet_mobilenet.tflite`
...
```bash
docker build --build-arg BUILD_TYPE=runtime -t aiha-posenet .
```
