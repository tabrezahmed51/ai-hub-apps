...
public class AIHubDefaults {
    ...
    public static final Set<TFLiteHelpers.DelegateType> enabledDelegates =
            new HashSet<>(
                    Arrays.asList(
    ...
    public static final TFLiteHelpers.DelegateType[][] delegatePriorityOrder =
            new TFLiteHelpers.DelegateType[][] {
                // 1. QNN_NPU + GPUv2 + XNNPack
                // https://app.aihub.qualcomm.com/docs/hub/api.html#profile-inference-options
                // Similar to AI Hub "compute_unit=all", or "compute_unit=npu,gpu,cpu" on QC devices
                // that
                // support QNN
                ...
                // ------
                //
                // Additional delegates (eg. NNAPI, or something targeting non-Qualcomm hardware)
                // could be
                // added here or above,
    ...
    // Create a version of the above delegate priority order that can only use the provided
    // delegates.
    public static TFLiteHelpers.DelegateType[][] delegatePriorityOrderForDelegates(
            Set<TFLiteHelpers.DelegateType> enabledDelegates) {
        return Arrays.stream(delegatePriorityOrder)
                .filter(x -> enabledDelegates.containsAll(Arrays.asList(x)))
