...
public class TFLiteHelpers {
    ...
    public enum DelegateType {
        ...
        //
        //   * Floating point compute is supported only on Snapdragon 8 Gen 1 and newer (with some
        // exceptions).
        //     CreateInterpreterAndDelegatesFromOptions will not apply this delegate for floating
        // point
        // models on hardware that does not support them.
        //     See documentation for hardware support details
        // https://docs.qualcomm.com/bundle/publicresource/topics/80-63442-50/overview.html
        QNN_NPU,

        // ------
        //
        // Additional delegates (eg. NNAPI, or something targeting non-Qualcomm hardware) could be
        // added
        // here.
        //
        // ------
     ...
     * @param delegatePriorityOrder Delegates, in order they should be registered to the
     *     interpreter.
     *     <p>The "inner array" defines which delegates should be registered when creating the
     *     interpreter. The order of delegates is the priority in which they are assigned layers.
     *     For example, if an array contains delegates { QNN_NPU, GPUv2 }, then QNN_NPU will be
     *     assigned any compatible op first. GPUv2 will then be assigned any ops that QNN_NPU is
     *     unable to run. And finally, XNNPack will be assigned ops that both QNN_NPU and GPUv2 are
     *     unable to run.
     *     <p>The "outer array" defines the order of delegate list the interpreter should be created
     *     with. This method will first attempt to create an interpreter using all delegates in the
     *     first array. If that interpreter fails to instantiate, this method will try to create an
     *     interpreter using all delegates in the second array. This continues until an interpreter
     *     could be successfully created & returned, or until all arrays are tried
     *     unsuccessfully--which results in an exception.
     ...
     * @return A pair of the created interpreter and associated delegates. These delegates must be
     *     kept in memory until they are no longer needed. Before deleting, the client must call
     *     close() on the returned delegates and interpreter.
     */
    public static Pair<Interpreter, Map<DelegateType, Delegate>>
        ...
        for (DelegateType[] delegatesToRegister : delegatePriorityOrder) {
            ...
            delegates.keySet().stream()
                    .filter(
                            delegateType ->
                                    Arrays.stream(delegatesToRegister)
                                            .noneMatch(d -> d == delegateType))
                    .collect(Collectors.toSet()) // needed so we don't modify the same object we're
                    // looping over
                    .forEach(
                            unusedDelegateType -> {
                                Objects.requireNonNull(delegates.remove(unusedDelegateType))
                                        .close();
                            });
        ...
        throw new RuntimeException(
                "Unable to create an interpreter of any kind for the provided model. See log for"
                        + " details.");
    }
     ...
     *
     * @param delegates Delegate instances to be registered in the interpreter. Delegates will be
     *     registered in the order of this array.
     * @param numCPUThreads Number of CPU threads to use for layers on CPU.
    ...
    public static Interpreter CreateInterpreterFromDelegates(
        ...
        tfLiteOptions.setUseXNNPACK(
                true); // Fall back to XNNPack (fast CPU implementation) if a layer cannot run on
        // NPU.
     ...
     * @return The loaded model in MappedByteBuffer format, and a unique model identifier hash
     *     string.
    ...
    static Delegate CreateDelegate(
        ...
        throw new RuntimeException(
                "Delegate creation not implemented for type: " + delegateType.name());
    }

    /**
     * Create and configure the QNN NPU delegate. QNN NPU will be configured for maximum performance
     * (at the cost of device battery life / heat / precision).
     *
     * @param nativeLibraryDir Native library directory for Android app.
     * @param cacheDir Android app cache directory.
     * @param modelIdentifier Unique identifier string for the model being loaded.
     * @return The created delegate if successful, and null otherwise.
     */
    static Delegate CreateQNN_NPUDelegate(
            String nativeLibraryDir, String cacheDir, String modelIdentifier) {
        ...
        //
        // dsp_options.performance_mode -->
        // qnnOptions.setDspOptions(QnnDelegate.Options.DspPerformanceMode.<PERF_MODE>,
        //
        // QnnDelegate.Options.DspPdSession.DSP_PD_SESSION_ADAPTIVE);
        //
        // htp_options.performance_mode -->
        // qnnOptions.setHtpPerformanceMode(QnnDelegate.Options.HtpPerformanceMode.<PERF_MODE>);
        // htp_options.precision -->
        // qnnOptions.setHtpPrecision(QnnDelegate.Options.HtpPrecision.<PRECISION>);
        // htp_options.useConvHmx -->
        // qnnOptions.setHtpUseConvHmx(QnnDelegate.Options.HtpUseConvHmx.HTP_CONV_HMX_<ON/OFF>);
        //
        // -------------------------------
    ...
    /**
     * Create and configure the GPUv2 delegate. GPUv2 will be configured for maximum performance (at
     * the cost of device battery life / heat / precision), and to allow execution in FP16
     * precision.
     *
     * @param cacheDir Android app cache directory.
     * @param modelIdentifier Unique identifier string for the model being loaded.
    ...
    static Delegate CreateGPUv2Delegate(String cacheDir, String modelIdentifier) {
        ...
        // gpuOptions.setInferencePreference(GpuDelegateFactory.Options.<PREFERENCE>)
        //
        ...
        //
        //                             If TFLITE_GPU_INFERENCE_PRIORITY_MAX_PRECISION is priority 2
        // or
        // 3, set
