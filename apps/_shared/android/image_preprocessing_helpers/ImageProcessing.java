...
public class ImageProcessing {
    /**
     * Resize a bitmap while respecting its aspect ratio. If the output image cannot fit perfectly
     * within the requested output height / width, padding is added such that the output bitmap will
    ...
    public static Bitmap resizeAndPadMaintainAspectRatio(
            Bitmap image, int outputBitmapWidth, int outputBitmapHeight, int paddingValue) {
        ...
        Bitmap outputImage =
                Bitmap.createBitmap(outputBitmapWidth, outputBitmapHeight, Bitmap.Config.ARGB_8888);
        ...
        can.drawBitmap(
                image, null, new RectF(left, top, finalWidth + left, finalHeight + top), null);
        return outputImage;
    }
}
