...
public class SuperResolution implements AutoCloseable {
     ...
     */
    public SuperResolution(Context context, String modelPath)
    ...
    public SuperResolution(
        ...
        assert inputType == DataType.UINT8
                || inputType == DataType.FLOAT32; // UINT8 (Quantized) and FP32 Input Supported
        ...
        // Set-up preprocessor
        inputImageProcessor =
                new ImageProcessor.Builder().add(new NormalizeOp(0.0f, 255.0f)).build();
    ...
    private ByteBuffer[] preprocess(Bitmap image) {
        ...
        if (image.getHeight() > inputShape[1] || image.getWidth() > inputShape[2]) {
            ...
            // While this app could easily resize the large image to fit, that defeats the purpose
            // of
            // super resolution.
            throw new RuntimeException(
                    "Input image is too big for this model. Expected Width of "
                            + inputShape[1]
                            + " and Height of "
                            + inputShape[2]);
            ...
            resizedImg =
                    ImageProcessing.resizeAndPadMaintainAspectRatio(
                            image, inputShape[1], inputShape[2], 0xFF);
        } else {
            resizedImg = image;
        }
